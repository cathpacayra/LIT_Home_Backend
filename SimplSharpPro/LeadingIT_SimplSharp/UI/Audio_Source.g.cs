...
{
    public interface IAudio_Source
    {
        ...
        void Is_Selected(Audio_SourceBoolInputSigDelegate callback);
        void Dig_Reserved_Out1(Audio_SourceBoolInputSigDelegate callback);
        void Name(Audio_SourceStringInputSigDelegate callback);

    }

    public delegate void Audio_SourceBoolInputSigDelegate(BoolInputSig boolInputSig, IAudio_Source audio_Source);
    public delegate void Audio_SourceStringInputSigDelegate(StringInputSig stringInputSig, IAudio_Source audio_Source);

    internal class Audio_Source : IAudio_Source, IDisposable
    {
        ...
        internal Audio_Source(ComponentMediator componentMediator, uint controlJoinId)
        {
        ...
        public void Is_Selected(Audio_SourceBoolInputSigDelegate callback)
        {
        ...
        public void Dig_Reserved_Out1(Audio_SourceBoolInputSigDelegate callback)
        {
            for (int index = 0; index < Devices.Count; index++)
            {
        ...
        public void Name(Audio_SourceStringInputSigDelegate callback)
        {
            for (int index = 0; index < Devices.Count; index++)
            {
        ...
        {
            return string.Format("Contract: {0} Component: {1} HashCode: {2} {3}", "Audio_Source", GetType().Name, GetHashCode(), UserObject != null ? "UserObject: " + UserObject : null);
        }
